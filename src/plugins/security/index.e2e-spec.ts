...
describe(`${CertificateAndKeyManagement.name} and ${
  ...
  describe(IdentityProvider.name, () => {
  ...
  describe(CertificateAndKeyManagement.name, () => {
    it('should create a self-signed certificate', function() {
      this.timeout(1000 * 60);
      this.slow(1000 * 15);
      const cmd = child.spawnSync(path.resolve('bin', 'run'), [
        'browserforce:apply',
  ...
  describe(IdentityProvider.name, () => {
    it('should enable identity provider with existing Certificate', function() {
      this.timeout(1000 * 60);
      this.slow(1000 * 15);
      const cmd = child.spawnSync(path.resolve('bin', 'run'), [
        'browserforce:apply',
