...
export class AuthenticationConfiguration extends BrowserforcePlugin {
  public async retrieve(definition: Config): Promise<Config> {
    ...
    const services = await frameOrPage.locator(SERVICE_CHECKBOX_SELECTOR).evaluateAll(
  ...
  public async apply(plan: Config): Promise<void> {
    ...
    for (const svc of plan.services) {
      const checkboxId = (await frameOrPage.locator(SERVICE_CHECKBOX_SELECTOR).evaluateAll((inputs, serviceName) => {
