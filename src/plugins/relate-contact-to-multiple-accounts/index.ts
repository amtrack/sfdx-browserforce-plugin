...
export class RelateContactToMultipleAccounts extends BrowserforcePlugin {
  public async retrieve(definition?: Config): Promise<Config> {
    ...
    const response = {
      enabled: await page.$eval(ENABLED_SELECTOR, (el: HTMLInputElement) => el.checked),
    };
    await page.close();
    return response;
  }
  ...
  public async apply(config: Config): Promise<void> {
    ...
    await Promise.all([page.waitForNavigation(), page.click(EDIT_BUTTON_SELECTOR)]);
    ...
    if (config.enabled) {
      ...
      await Promise.all([page.waitForNavigation(), page.click(DISABLE_CONFIRM_BUTTON_SELECTOR)]);
    }
