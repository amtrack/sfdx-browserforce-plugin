...
async function createBrowserContextFromEnv() {
  const browser = await chromium.launch({
    ...(process.env.BROWSERFORCE_BROWSER_CHANNEL
      ? {
          channel: process.env.BROWSERFORCE_BROWSER_CHANNEL,
        }
      : {}),
    ...(process.env.CHROME_BIN
      ...
      : {}),
    ...(process.env.BROWSERFORCE_BROWSER_EXECUTABLE_PATH
      ? {
          executablePath: process.env.BROWSERFORCE_BROWSER_EXECUTABLE_PATH,
        }
      : {}),
  ...
  if (process.env.BROWSERFORCE_TRACE === 'true') {
